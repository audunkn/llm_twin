...
# The embedding model class implements the singleton pattern (https://refactoring.guru/design-patterns/singleton), 
# a creational design pattern that ensures a class has only one instance while providing a global access point to this instance. 
# The EmbeddingModelSingleton() class inherits from the SingletonMeta class, which ensures that whenever an EmbeddingModelSingleton() is instantiated, 
# it returns the same instance. This works well with ML models, as you load them once in memory through the singleton pattern, 
# and afterward, you can use them anywhere in the code base. Otherwise, you risk loading the model in memory every time you use it or loading it multiple times,
#  resulting in memory issues. Also, this makes it very convenient to access properties such as embedding_size, 
# where you have to make a dummy forward pass into the embedding model to find the size of its output. 
# As a singleton, you do this forward pass only once, and then you have it accessible all the time during the program’s execution.
