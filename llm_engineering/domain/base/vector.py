...
# All operations made to Qdrant will be done through the connection instance, which is instantiated in the application’s infrastructure layer
from llm_engineering.infrastructure.db.qdrant import connection

T = TypeVar("T", bound="VectorBaseDocument")


# The VectorBaseDocument class inherits from Pydantic’s BaseModel and helps us structure a single record’s attributes from the vector DB. 
# Every OVM will be initialized by default with UUID4 as its unique identifier. 
# Using generics—more precisely, by inheriting from Generic[T]—the signatures of all the subclasses of the VectorBaseDocument class 
# will adapt to that given class. For example, the from_record() method of the Chunk() class, which inherits VectorBaseDocument, 
...
class VectorBaseDocument(BaseModel, Generic[T], ABC):
    ...
    # On the other hand, the to_point() method takes the attributes of the current instance and adapts them to Qdrant’s PointStruct() format.
